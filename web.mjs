...
const TEMPLATE_DATA = [
	...
	},
	{
		...
		video: 'https://www.youtube.com/embed/sm-oWq1UAjA?si=kzZKt2-z3PessviL',
		image: '/m/a/t/6.svg'
	}
];
...
const PORTFOLIO_DATA = [
	...
	},
	{
		id: 2,
		title: '주간보고 시스템 구축',
		description: '주차 계산 로직을 적용해 반복 업무를 줄이고, 누구나 동일한 기준으로 리포트를 작성할 수 있도록 정리했습니다.',
...
async function loadPagePart(pagePartName, targetElement, addHistory = true) {
	try
	{
		...
		if (pagePartName === 'templatelist' || pagePartName === 'portfoliolist') {
			const waitForComponent = async () => {
				const component = document.querySelector('card-component');
				if (component && typeof component.filterCards === 'function') {
					await component.ready;
